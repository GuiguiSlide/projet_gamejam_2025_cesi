...
@export var tower_scenes : Array = []
const SHOOT_COOLDOWN = 0.3
const SCROLL_COOLDOWN = 1.0 # 10 seconds between scrolls
...
func _ready():
	...
	# Setup scroll cooldown timer
	scroll_cooldown_timer = Timer.new()
	scroll_cooldown_timer.wait_time = SCROLL_COOLDOWN
	scroll_cooldown_timer.one_shot = true
	scroll_cooldown_timer.connect("timeout", Callable(self, "_on_scroll_cooldown_timeout"))
	...
	main_music_player.volume_db = -20.0
...
func _input(event):
	...
	if current_weapon == "wrench" and can_scroll:
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_WHEEL_UP:
				...
				_start_scroll_cooldown()
			elif event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
				current_tower_index = (current_tower_index + 1) % tower_scenes.size()
				print("Selected tower index: ", current_tower_index)
				_start_scroll_cooldown()

func _start_scroll_cooldown():
	can_scroll = false
	scroll_cooldown_timer.start()

func _on_scroll_cooldown_timeout():
	can_scroll = true
